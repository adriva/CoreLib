	...
	public class QueueMessage {
		...
		public QueueMessage(QueueMessageFlags flags, string commandType, object data, string lockIdentifier) {
			...
			if (string.IsNullOrWhiteSpace(this.LockIdentifier)) {
				// try to get the instance from the web job hosting environment
				// this is not set in dev env so use the machine name
				this.LockIdentifier = Environment.GetEnvironmentVariable("WEBSITE_INSTANCE_ID");
